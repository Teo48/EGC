...
void main()
{
	...
	if (Color == 1) {
		gl_Position = Projection * View * Model * vec4(v_position + v_normal * cnoise(
			(sin(Clock * 5.f) * sin(Clock * 5.f) * sin(Clock * 5.f)* cos(Clock * 5.f) - 
			sin(Clock * 5.f) * cos(Clock * 5.f)) *v_position), 1.0);
	} else if (Color == 0) {
		gl_Position = Projection * View * Model * vec4(v_position + v_normal * snoise(sin(Clock * 2.f) * cos(Clock * 2.f) * v_position), 1.0);
	}
}
